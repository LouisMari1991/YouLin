...
public class IndexBar extends View {
  ...
  private void init(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    mPaint.setColor(ActivityCompat.getColor(getContext(), R.color.text_deep_light));
  ...
  @Override protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    switch (wMode) {
      case MeasureSpec.EXACTLY: // match_parent 或者 固定的大小
        measureWidth = wSize;
        break;
  ...
  @Override protected void onDraw(Canvas canvas) {
    ...
    for (int i = 0; i < mIndexDatas.size(); i++) {
      index = mIndexDatas.get(i);
      if (i == passIndex) {
        mPaint.setColor(ActivityCompat.getColor(getContext(), R.color.text_black));
      } else {
        mPaint.setColor(ActivityCompat.getColor(getContext(), R.color.text_deep_light));
      }
      Paint.FontMetrics fontMetrics =
          mPaint.getFontMetrics();//获得画笔的FontMetrics，用来计算baseLine。因为drawText的y坐标，代表的是绘制的文字的baseLine的位置
      int baseline = (int) ((mGapHeight - fontMetrics.bottom - fontMetrics.top)
          / 2);//计算出在每格index区域，竖直居中的baseLine值
      canvas.drawText(index, mWidth / 2 - mPaint.measureText(index) / 2,
          t + mGapHeight * i + baseline, mPaint);//调用drawText，居中显示绘制index
    }
  }
  ...
  public IndexBar setmSourceDatas(List<? extends ISuspensionInterface> mSourceDatas) {
  ...
  private int getPosByTag(String tag) {
    ...
    for (int i = 0; i < mSourceDatas.size(); i++) {
      if (tag.equals(mSourceDatas.get(i).getInitialLetter())) {
