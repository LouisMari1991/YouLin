...
public class SuspensionDecoration extends RecyclerView.ItemDecoration {
  ...
  private int drawLiftMargin; // 默认左边距为10dp
  ...
  public SuspensionDecoration(Context context, List<? extends ISuspensionInterface> datas) {
    ...
    drawLiftMargin = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 10,
        context.getResources().getDisplayMetrics());
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if (position > -1) {
        if (position == 0) {//等于0肯定要有title的
          ...
          if (null != mDatas.get(position).getInitialLetter() && !mDatas.get(position)
              .getInitialLetter()
              .equals(mDatas.get(position - 1).getInitialLetter())) {
...
/*
    ...
    mPaint.getTextBounds(mDatas.get(position).getInitialLetter(), 0,
        mDatas.get(position).getInitialLetter().length(), mBounds);
    c.drawText(mDatas.get(position).getInitialLetter(), child.getPaddingLeft() + drawLiftMargin,
  ...
  @Override public void getItemOffsets(Rect outRect, View view, RecyclerView parent,
    ...
    if (mDatas == null
        || mDatas.isEmpty()
        || position > mDatas.size() - 1) {//pos为1，size为1，1>0? true
      return;//越界
    }
    //我记得Rv的item position在重置时可能为-1.保险点判断一下吧
    if (position > -1) {
      ...
      if (position == 0) {
        ...
        if (null != titleCategoryInterface.getInitialLetter()
            && !titleCategoryInterface.getInitialLetter()
            .equals(mDatas.get(position - 1).getInitialLetter())) {
          //不为空 且跟前一个tag不一样了，说明是新的分类，也要title
          outRect.set(0, mTitleHeight, 0, 0);
        }
