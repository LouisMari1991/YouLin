...
public class SwipeMenuLayout extends ViewGroup {
  ...
  /**
   * 联系人列表右侧默认滑动距离,为了处理联系人列表索引挡住button的文字
   * 2017/8/28 add
   */
  ...
  private void init(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      } else if (attr == R.styleable.SwipeMenuLayout_slide_right_margin) {
        slideRightMargin = ta.getDimensionPixelSize(attr, 0);
      }
    }
    ta.recycle();
  }
  ...
  @Override protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    //mRightMenuWidths = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 35,
    //    getContext().getResources().getDisplayMetrics());//由于ViewHolder的复用机制，每次这里要手动恢复初始值
    mRightMenuWidths = slideRightMargin;
    mHeight = 0;
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if (childView.getVisibility() != GONE) {
        ...
        mHeight =
            Math.max(mHeight, childView.getMeasuredHeight()/* + lp.topMargin + lp.bottomMargin*/);
...
/*                    if (!mScroller.isFinished()) {
  ...
  public void smoothExpand() {
    ...
    cancelAnim();
    mExpandAnim =
        ValueAnimator.ofInt(getScrollX(), isLeftSwipe ? mRightMenuWidths : -mRightMenuWidths);
