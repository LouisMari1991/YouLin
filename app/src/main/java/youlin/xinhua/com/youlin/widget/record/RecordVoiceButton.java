...
public class RecordVoiceButton extends ImageButton {
  ...
  public void setVoiceFilePath(String path, String fileName) {
    if (null == path || TextUtils.isEmpty(path) || null == fileName || TextUtils.isEmpty(
        fileName)) {
  ...
  public void finishRecord(boolean isPreview) {
    ...
    if (intervalTime < MIN_INTERVAL_TIME) {
      Toast.makeText(getContext(), mContext.getString(R.string.time_too_short_toast),
          Toast.LENGTH_SHORT).show();
  ...
  private void startRecording() {
    try {
      ...
      e.printStackTrace();
      Toast.makeText(mContext, mContext.getString(R.string.illegal_state_toast), Toast.LENGTH_SHORT)
          .show();
      cancelTimer();
      dismissDialog();
      if (mThread != null) {
